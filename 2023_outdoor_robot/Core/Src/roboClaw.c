...
ROBOCLAW_StatusTypeDef SetMotor1DefaultSpeed(RoboClaw_HandleTypeDef* hroboClaw,uint16_t speed){
	return write_n(hroboClaw, 4, hroboClaw->packetserial_address,SETM1EDFAULTSPEED,SetWORDval(speed));
}

ROBOCLAW_StatusTypeDef SetMotor2DefaultSpeed(RoboClaw_HandleTypeDef* hroboClaw,uint16_t speed){
	return write_n(hroboClaw, 4, hroboClaw->packetserial_address,SETM2EDFAULTSPEED,SetWORDval(speed));
}

ROBOCLAW_StatusTypeDef DriveM1withSpeedandPosition(RoboClaw_HandleTypeDef* hroboClaw, int32_t speed, int32_t Position){
	return write_n(hroboClaw, 11, hroboClaw->packetserial_address,M1SPEEDPOSITION, SetDWORDval(speed), SetDWORDval(Position));
}
...
//ROBOCLAW_StatusTypeDef DriveM2withSpeedandPosition(RoboClaw_HandleTypeDef* hroboClaw, uint32_t speed, uint32_t Position){
//	return write_n(hroboClaw, 11, hroboClaw->packetserial_address,M2SPEEDPOSITION, SpeedM2(hroboClaw,speed), SetDWORDval(Position));
//}

ROBOCLAW_StatusTypeDef DriveM1withPosition(RoboClaw_HandleTypeDef* hroboClaw,int32_t M1Position){
	return write_n(hroboClaw, 7, hroboClaw->packetserial_address,DRIVEM1WITHPOSITION,SetDWORDval(M1Position));
}

ROBOCLAW_StatusTypeDef DriveM2withPosition(RoboClaw_HandleTypeDef* hroboClaw,int32_t M2Position){
	return write_n(hroboClaw, 7, hroboClaw->packetserial_address,DRIVEM2WITHPOSITION,SetDWORDval(M2Position));
}

ROBOCLAW_StatusTypeDef DriveM1M2WithPosition(RoboClaw_HandleTypeDef* hroboClaw, int32_t M1Position, int32_t M2Position){
	return write_n(hroboClaw, 11, hroboClaw->packetserial_address,DRIVEM1M2WITHPOSITION,SetDWORDval(M1Position),SetDWORDval(M2Position));
}
...
void GETENCODERSM1M2(RoboClaw_HandleTypeDef* hroboClaw){
//	HAL_UART_Transmit_DMA(&huart5,listC,sizeof(listC));
	write_n(hroboClaw,2, hroboClaw->packetserial_address,GETENCODERS);
...
//이명욱이 만든거
uint32_t ReadEncmode(RoboClaw_HandleTypeDef* hroboClaw, uint8_t *status, bool *valid){
	return Read4_1(hroboClaw, GETENCODERMODE, status, valid);
}
